...
class QLearning:
    def __init__(self, agentsProfileName="kochi/data/agentsdb.csv",
        ...
        # changed 2022.01
        # [nodeInit, X slots for links density code, X slots for action-value functions, \n
        # X slots for counting the observations of this states]
        self.slots = self.transLinkdb.shape[1]
        self.stateMat = np.zeros(( self.nodesdb.shape[0], (self.slots * 3 + 1)))
        ...
        for i in range(self.nodesdb.shape[0]):
            self.stateMat[i, self.slots + 1: self.slots + 1 + int(self.transLinkdb[i, 1])] = 0.5 * np.ones(self.transLinkdb[i, 1])
            self.stateMat[i, 2 * self.slots + 1: 2 * self.slots + 1 + int(self.transLinkdb[i, 1])] = np.ones(self.transLinkdb[i, 1])
    ...
    def getStateIndexAtNode(self, codeNode):
        ...
        state_arr = np.zeros(3 * self.slots + 1)
        ...
        for i in indx[0]:
            if np.array_equal(state_arr[:self.slots + 1],self.stateMat[i,:self.slots + 1]):
                return i
        # If the state at node "codeNode" does not exist, it will create a new state at botttom
        state_arr[self.slots + 1: self.slots + 1 + linksdb[1]] = 0.5*np.ones(linksdb[1])
        state_arr[2 * self.slots + 1: 2 * self.slots + 1 + linksdb[1]] = np.ones(linksdb[1])
        self.stateMat = np.concatenate((self.stateMat, state_arr.reshape(1, 3 * self.slots + 1)), axis=0)
    ...
    def exportStateMatrix(self, outnamefile="stateMatrix.csv"):
        ...
        """
        fmt = ["%d"]+["%d"]*self.slots+["%.6f"]*self.slots+["%d"]*self.slots
        np.savetxt(outnamefile, self.stateMat, delimiter=',', fmt=fmt)
        return
...
#            print("high density of %.4f at %d" % (density,codeLink))
    ...
    def updateTarget(self, pedIndx, ifOptChoice = False):
        ...
        # Otherwise, we assign new target node:
            ...
            Qval_arr = self.stateMat[stateIndx , self.slots+1:self.slots+1+int(self.transLinkdb[node0,1])]
    ...
    def tdControl(self, pedIndx, alpha= 0.05):
        ...
        if self.pedDB[pedIndx,10]:
            ...
            self.stateMat[current_S, self.slots + 1 + current_A] += alpha * (currentReward - self.stateMat[current_S, self.slots + 1 + current_A])
            self.stateMat[current_S, 2 * self.slots + 1 + current_A] += 1
        
        preReward= self.stepReward * (current_t - pre_t)
        self.stateMat[pre_S, self.slots + 1 + pre_A] += alpha * (preReward + self.discount * self.stateMat[current_S, self.slots + 1 + current_A] - self.stateMat[pre_S, self.slots + 1 + pre_A])
        self.stateMat[pre_S, 2 * self.slots + 1 + pre_A] += 1
        return
    ...
    def updateValuefunctionByAgent(self, pedIndx):
        ...
        self.stateMat[int(expSta[0,0]), int(self.slots + 1 + expSta[0,1])] += (reward - self.stateMat[int(expSta[0,0]), int(self.slots + 1 + expSta[0,1])])/(self.stateMat[int(expSta[0,0]), 2 * self.slots + 1 + int(expSta[0,1])] + 1)
        self.stateMat[int(expSta[0,0]), 2 * self.slots + 1 + int(expSta[0,1])] += 1
        if len(expSta) > 1:
            for i in range(1,expSta.shape[0]):
                if expSta[i,0] in expSta[:i,0]:
                    continue
                self.stateMat[int(expSta[i,0]), self.slots + 1 + int(expSta[i,1])] += (reward - self.stateMat[int(expSta[i,0]), self.slots + 1 + int(expSta[i,1])])/(self.stateMat[int(expSta[i,0]), 2 * self.slots + 1 + int(expSta[i,1])] + 1)
                self.stateMat[int(expSta[i,0]), 2 * self.slots + 1 + int(expSta[i,1])] += 1    
        return 
    ...
    def updateValuefunctionByAgentV2(self, pedIndx, ifConstStepSize= True, alpha= 0.05):
        ...
        if ifConstStepSize:
            self.stateMat[state, self.slots + 1 + choice] += alpha*(G - self.stateMat[state, self.slots + 1 + choice])
            self.stateMat[state, 2 * self.slots + 1 + choice] += 1
        else:
            self.stateMat[state, self.slots + 1 + choice] += (G - self.stateMat[state, self.slots + 1 + choice])/(self.stateMat[state, 2 * self.slots + 1 + choice] + 1)
            self.stateMat[state, 2 * self.slots + 1 + choice] += 1
    ...
    def computeAction_Value_Policy(self):
        ...
        """
        # Get action values
        QFun = np.zeros(( self.stateMat.shape[0] , self.slots))
        QFun = self.stateMat[:,self.slots + 1: 2 * self.slots + 1]
        # Get policy
        ...
        # Get value function
        VFun = np.sum( self.stateMat[ : , self.slots + 1:2 * self.slots + 1 ] * self.stateMat[ : , 2 * self.slots + 1:3 * self.slots + 1 ] , axis = 1) / np.sum(self.stateMat[ : , 2 * self.slots + 1:3 * self.slots + 1 ] , axis=1) 

...
def getPrefEvacBldgs(crs='EPSG:4326'):
    ...
    bldgs = gpd.GeoDataFrame(bldgs_pd, geometry=gpd.points_from_xy(bldgs_pd['lon'], bldgs_pd['lat']))
    bldgs.set_crs('EPSG:4326', inplace=True)
    bldgs.to_crs(crs)
    return bldgs


def getAreaEvacBldgs(area={'north': 33.58, 'south': 33.53, 'east': 133.58, 'west': 133.52}, crs='EPSG:6690'):
    bbox = createBoundingBox(area)
    bbox_gdf = gpd.GeoSeries(bbox)
    bbox_gdf.set_crs('EPSG:4326', inplace=True)
    bbox_gdf = bbox_gdf.to_crs(crs)
...
def fixLinksDBAndNodesDB(shelters):
    ...
    np.savetxt("./data/nodesdb.csv", nodesdb, delimiter=",", header="number,coord_x,coord_y,evacuation,reward",
               fmt="%d,%.6f,%.6f,%d,%d")
    np.savetxt("./data/linksdb.csv", linksdb, delimiter=",", header="number,node1,node2,length,width",
               fmt="%d,%d,%d,%d,%d")
    return
...
def copyDataFolder(case='kochi'):
    shutil.copytree(Path('./data'), Path(f'../{case}/data'))

...
def dataBaseBy4thOrderMesh(bldShp, meshShp, meshCode, outBldDb_path, outCensusDb_path):
    ...
    outfileCenso = open(census_file, 'w')   # creating output file with censo data for disaggregation
    ...
    outfileBld = open(bld_file, 'w')   # creating output file with building database
    outfileBld.write("BldCode,CenterX,CenterY,Height,Area,Mesh100MCode\n")

    
    
    targetSpaRef = osr.SpatialReference()
    targetSpaRef.ImportFromEPSG(32654) # UTM zone 54N
    ...
    for featureMesh in layerMesh:
        ...
        sourceSpaRef = layerBld.GetSpatialRef()    # getting the spatial reference of the shapefile 
        src2trg = osr.CoordinateTransformation(sourceSpaRef, targetSpaRef)    # transformation function
...
def createCensusAndPopDatabase(bldShp, meshShp, outBldDb_path, outCensusDb_path, fileOriginalDataSource):
    
    dataSubmeshCode = np.loadtxt(fileOriginalDataSource.decode("utf-8"), skiprows=1, usecols=(3,), delimiter = '"', dtype = np.str)
...
#    print(submeshCodeArray[0], len(submeshCodeArray), type(submeshCodeArray))
    for sm in submeshCodeArray:
        print(sm)
        dataBaseBy4thOrderMesh(bldShp, meshShp, sm, outBldDb_path, outCensusDb_path)
